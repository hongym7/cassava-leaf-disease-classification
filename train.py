...
for fold, (train_idx, val_idx) in enumerate(skf.split(range(len(FILENAMES)))):
    ...
    base_model = efn.EfficientNetB3(weights='noisy-student', include_top=False, input_shape=[*IMAGE_SIZE, 3])
    ...
    checkpoint = tf.keras.callbacks.ModelCheckpoint('ckpt/effB3-fold-%i.h5' % fold,
