...
for fold, (train_idx, val_idx) in enumerate(skf.split(range(len(FILENAMES)))):
    ...
    #base_model = efn.EfficientNetB5(weights='noisy-student', include_top=False, input_shape=[*IMAGE_SIZE, 3])
    base_model = se_resnext.SEResNextImageNet(input_shape=[*IMAGE_SIZE, 3], include_top=False, weights='imagenet', classes=5)
    ...
    checkpoint = tf.keras.callbacks.ModelCheckpoint('ckpt/seresnext-fold-%i.h5' % fold,
    ...
    model1.load_weights('ckpt/seresnext-fold-%i.h5' % fold)
